...
class DialogueCharacter extends FlxSprite
{
	...
	public function new(x:Float = 0, y:Float = 0, character:String = null)
	{
		...
		frames = Paths.getSparrowAtlas('game/dialogue/characters/' + jsonFile.image);
		reloadAnimations();
	...
	public function reloadCharacterJson(character:String) {
		var characterPath:String = 'images/game/dialogue/characters/' + character + '.json';
		var rawJson = null;
		...
		if(!FileSystem.exists(path)) {
			path = Paths.getSharedPath('images/game/dialogue/characters/' + DEFAULT_CHARACTER + '.json');
		}
