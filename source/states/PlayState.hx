...
class PlayState extends MusicBeatState
{
	...
	//Gameplay settings
	public var cam_note_move_amt:Int = 20;
	var cam_displace_x:Float = 0;
	var cam_displace_y:Float = 0;
	...
	override public function create()
	{
		...
		Lib.application.window.title = "Torture's Torment | " + SONG.song + " (" + Difficulty.getString().toUpperCase() + ")";
		...
		switch (curStage)
		{
			case 'blank': new states.stages.Template(); // Blank stage
		...
		insert(members.indexOf(boyfriendGroup), comboGroup);
		add(noteGroup);
		add(uiGroup);
		...
		timeBar = new Bar(0, timeTxt.y + (timeTxt.height / 4), 'game/timeBar', function() return songPercent, 0, 1);
		...
		healthBar = new Bar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), 'game/healthBar', function() return health, 0, 2);
	...
	function cacheCountdown()
	{
		...
		var introImagesArray:Array<String> = switch(stageUI) {
			case "pixel": ['game/ready-pixel', 'game/set-pixel', 'game/date-pixel'];
			case "normal": ["game/ready", "game/set" ,"game/go"];
			default: ['game/ready', 'game/set', 'game/go'];
		}
	...
	{
		var str:String = "N/A"; /* Language.getPhrase('rating_$ratingName', ratingName); */
		if(totalPlayed != 0)
		{
			var percent:Float = CoolUtil.floorDecimal(ratingPercent * 100, 2);
			str = Language.getPhrase('percent', percent + '%');
		}

		var tempScore:String;
		if(!instakillOnMiss) tempScore = Language.getPhrase('score_text', '- Score: {1} | Accuracy: {2} | {3} -', [songScore, str, Language.getPhrase(ratingFC)]);
		else tempScore = Language.getPhrase('score_text_instakill', 'Score: {1} | {2}', [songScore, Language.getPhrase(ratingFC)]);
	...
	override public function update(elapsed:Float)
	{
		...
		if (cpuControlled && scoreTxt.text != "- BOTPLAY -") scoreTxt.text = "- BOTPLAY -";
		...
		updateIconScale(iconP1, elapsed); updateIconScale(iconP2, elapsed);
		updateIconsPosition();
	...
	public dynamic function updateIconScale(icon:HealthIcon, elapsed:Float)
	{
		var mult:Float = FlxMath.lerp(1, icon.scale.x, Math.exp(-elapsed * ((Conductor.bpm / 60) * 4) * playbackRate));
	...
	public function moveCamera(isDad:Bool)
	{
		if(isDad)
		{
			...
			camFollow.x += dad.cameraPosition[0] + opponentCameraOffset[0] + cam_displace_x;
			camFollow.y += dad.cameraPosition[1] + opponentCameraOffset[1] + cam_displace_y;
			tweenCamIn();
		}
		else
		{
			...
			camFollow.x -= boyfriend.cameraPosition[0] - boyfriendCameraOffset[0] + cam_displace_x;
			camFollow.y += boyfriend.cameraPosition[1] + boyfriendCameraOffset[1] + cam_displace_y;
	...
	{
		var uiPrefix:String = 'game/ratings/';
		var uiSuffix:String = '';
		if (stageUI != "normal")
		{
			uiPrefix = 'game/ratings/pixel/$stageUI/';
	...
	function strumCameraRoll(note:Note)
	{
		if (SONG.notes[curSection] != null && cam_note_move_amt != 0 && generatedMusic && !endingSong && !isCameraOnForcedPos)
		{
			cam_displace_x = 0;
			cam_displace_y = 0;
			...
			{
				if (strumLineNotes.members[0].animation.curAnim.name == 'confirm' && !note.noAnimation)
					cam_displace_x -= cam_note_move_amt;
				if (strumLineNotes.members[1].animation.curAnim.name == 'confirm' && !note.noAnimation)
					cam_displace_y += cam_note_move_amt;
				if (strumLineNotes.members[2].animation.curAnim.name == 'confirm' && !note.noAnimation)
					cam_displace_y -= cam_note_move_amt;
				if (strumLineNotes.members[3].animation.curAnim.name == 'confirm' && !note.noAnimation)
					cam_displace_x += cam_note_move_amt;
			} else {
				if (strumLineNotes.members[4].animation.curAnim.name == 'confirm' && !note.noAnimation)
					cam_displace_x += cam_note_move_amt;
				if (strumLineNotes.members[5].animation.curAnim.name == 'confirm' && !note.noAnimation)
					cam_displace_y += cam_note_move_amt;
				if (strumLineNotes.members[6].animation.curAnim.name == 'confirm' && !note.noAnimation)
					cam_displace_y -= cam_note_move_amt;
				if (strumLineNotes.members[7].animation.curAnim.name == 'confirm' && !note.noAnimation)
					cam_displace_x -= cam_note_move_amt;
			}
			...
			moveCamera(SONG.notes[curSection].mustHitSection != true);
		}
	}
	...
	override function destroy() {
		...
		Lib.application.window.title = "Torture's Torment";
